...
class Execute(object, metaclass=FunctionName):
    ...
    def patent_clue_1_1(self):
        all_clue_type = [u'专利布局规划', u'研发立项支持', u'竞争对手监测', u'高价值专利培育', u'专利价值评估',
                         u'专利尽职调查', u'专利价值评估', u'专利侵权诉讼', u'优先审查', u'海外专利流程管理',
                         u'知识产权海关备案', u'植物新品种']
            ...
            try:
                if self.dboperate.is_member(self.db, clue_type):
                    ...
                    self.driver.find_element_by_xpath(
                        "(.//div[@class='ui-apply-zlsq']/div/ul/li[{}]/a)".format(num)).click()
                    time.sleep(0.5)
                    case_type = self.driver.find_element_by_xpath(
                        "(.//div[@class='ui-apply-zlsq']/div/ul/li[{}]/a)".format(num)).text
                    time.sleep(0.5)
    ...
    def patent_clue_5(self):
        all_clue_type = [u'双软认证', u'ISO9001质量管理体系认证', u'软件产品登记测试报告', u'科技成果评价',
                         u'贯标申请服务', u'知识产权保护', u'专利风险预警', u'专利侵权对抗', u'无效/异议提起',
                         u'合同审核', u'知识产权维权', u'咨询分析报告', u'知识产权运营', u'知识产权顾问']
        for clue_type in all_clue_type:
            if self.dboperate.is_member(self.db, clue_type):
                try:
                    locator = (By.XPATH, "(.//div[@class='fl isnaMar'])[2]")
            ...
            if self.dboperate.is_member(self.db, clue_type):
                try:
                    ...
                    price = self.driver.find_element_by_xpath(
                        "(.//ul[@class='soft-optaion fl']/label[{}]/li)".format(num)).text
                    time.sleep(0.5)
